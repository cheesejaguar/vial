	...
	{:else}
				...
				<div>
					...
					<span class="setting-value">
						{#if status && !status.locked}
							<span class="status-dot ok" style="display: inline-block; margin-right: 0.4rem;"></span>Unlocked
						{:else}
							<span class="status-dot danger" style="display: inline-block; margin-right: 0.4rem;"></span>Locked
						{/if}
					</span>
				</div>
				{#if status && !status.locked}
		...
		{#if config}
			<section class="section">
				...
				<p class="config-hint">Edit <code>~/.config/vial/config.yaml</code> to change these values.</p>
			</section>
		{/if}
...
<style>
	...
	h2 { font-size: 0.88rem; font-weight: 600; color: var(--text-secondary); margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.05em; }
	...
	.setting-label { font-size: 0.85rem; color: var(--text-secondary); display: block; }
	.setting-value { font-size: 0.85rem; color: var(--text-bright); display: flex; align-items: center; }

	.config-list { display: flex; flex-direction: column; gap: 1px; }
	.config-row { display: flex; justify-content: space-between; padding: 0.55rem 1rem; }
	.config-key { font-family: var(--font-mono); font-size: 0.82rem; color: var(--text-secondary); }
	.config-val { font-family: var(--font-mono); font-size: 0.82rem; color: var(--text-bright); }
	.config-hint { font-size: 0.75rem; color: var(--text-muted); margin-top: 0.5rem; }

	.about { padding: 1rem; font-size: 0.88rem; color: var(--text-secondary); }
	.about strong { color: var(--text-bright); }
	.about-links { margin-top: 0.5rem; font-size: 0.82rem; }
