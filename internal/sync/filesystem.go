...
// FilesystemBackend syncs the vault file via filesystem copy.
// Suitable for iCloud Drive, Dropbox, Google Drive, or any mounted filesystem.
type FilesystemBackend struct {
	remotePath string // full path to the remote vault file
}

// NewFilesystemBackend creates a filesystem sync backend.
...
// Push copies the local vault file to the remote path.
...
// LastModified returns the remote file's modification time.
...
// copyFile copies src to dst atomically (via temp file + rename).
